...
## 5) Create Application Constants and Enums

1. In the `src/app/models` folder, create `constants.ts`:
...
## 9) Create Standalone Components
...
Includes Material features:
- Material table with columns: Job Name, Status, Start, End, Duration, Actions
- Search input (job name)
- Status filter (MatSelect using APP_CONSTANTS)
- Status chips with color mapping (using StatusColorMap constant)
- Pagination with configurable page sizes [5, 10, 25, 50] items per page
- MatPaginator for improved data navigation
...
## 10) Ensure Material Modules are Imported in Each Component
